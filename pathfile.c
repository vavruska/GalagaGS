...
#ifdef USE_RESOURCES
/* Read one logical line from the in-memory level text starting at ptr,
 * accumulating '\\'-continued physical lines into buf.  Returns the number
 * of bytes consumed from ptr (including the terminating newline), 0 at end
 * of data, or -1 if the accumulated line is too long. */
int get_line(char *ptr, char *buf) {
    char *p = ptr;
    char *nl;
    int total = 0;
    int clen;
    ...
    for (;;) {
        nl = strchr(p, '\n');
        clen = nl ? (int)(nl - p) : (int)strlen(p);

        while (clen > 0 && p[clen - 1] == '\r') {
            clen--;
        }

        if ((int)strlen(buf) + clen >= MAXLINE) {
            return -1;
        }
        if (clen > 0) {
            int off = (int)strlen(buf);
            memcpy(buf + off, p, clen);
            buf[off + clen] = 0;
        }

        total += (nl ? clen + 1 : clen);
...
PathfileToken get_token(char *line, int *elem, char **data) {
    PathfileToken pt;
    char *datastart;
    ...
    *elem = -1;
    ...
    while (isspace(*datastart)) datastart++;
...
static void parse_path(int pnum, char *pstr) {
    char *fnd;
    char dir[40];
    int dp;
    int curdir = -1, gotdir;
    ...
    int sx, sy;
    char *comma;
    extern struct path_info pathinfo[];
    ...
    sy = atoi(comma + 1);
    fnd = strchr(pstr, ':');
    while (!iscompass(*fnd) && *fnd) fnd++;

    if (!(*fnd)) fprintf(stderr, "No path info for path %d\n", pnum);
    ...
    while (*fnd) {
        ...
        epaths[pnum][curentry].duration = duration/1.5;
        curentry++;
        while (isdigit(*fnd) || isspace(*fnd)) fnd++;
    }
...
static int parse_shapes(char *data) {
    char *chk;
    int i;
...
static int parse_paths(char *data) {
    char *chk;
    int i;
...
static int parse_delays(char *data) {
    char *chk;
    int i;
...
int read_level(int lev) {
    ...
    Handle lh = LoadResource(rText, lev);
    if (toolerror() || !lh) {
        return 0;
    }
    DetachResource(rText, lev);
...
#ifdef USE_RESOURCES
    char *ptr = *lh;
    int size = GetHandleSize(lh);
    while (size > 0) {
        rlen = get_line(ptr, readline);
        if (rlen <= 0) {
            if (rlen == 0) {
    ...
    while (!feof(lf)) {
        if ((rlen = get_line(lf, readline)) <= 0) {
            ...
            if (!gotShapes) fprintf(stderr, "  Missing shapes\n");
...
/* Fill out[0..5] with the alien shape value of each formation row, top to bottom. */
void level_alien_rows(int out[6]) {
    int r, c;

    for (r = 0; r < 6; r++) {
        out[r] = 0;
        for (c = 0; c < 10; c++) {
            if (al_shapes[r * 10 + c] >= 0) {
                out[r] = al_shapes[r * 10 + c];
                break;
            }
        }
    }
